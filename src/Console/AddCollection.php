...
class AddCollection extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        foreach ($directories as $directory) {
            ...
            $rootFiles = Filesystem::files($directory);

            foreach ($rootFiles as $rootFile) {
                $rootFilePath = 'app/templates/styleguide/' . basename($directory) . '/' . basename($rootFile);
                if (!Filesystem::exists($rootFilePath)) {
                    $output->writeLine('move: ' . $rootFilePath);
                    Filesystem::move($rootFile, $rootFilePath);
                }
            }
        }
